<!-- filepath: c:\laragon\www\toko-online\resources\views\penjual\produk\create.blade.php -->
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('penjual.produk.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_produk" value="{{ old('nama_produk') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('nama_produk') border-red-500 @enderror">
                        @error('nama_produk')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="harga" value="{{ old('harga') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('harga') border-red-500 @enderror">
                        @error('harga')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="stok" value="{{ old('stok', 0) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('stok') border-red-500 @enderror">
                        @error('stok')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
